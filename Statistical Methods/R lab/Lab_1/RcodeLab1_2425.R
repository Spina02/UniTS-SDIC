...
#                               #
# This is the post-LAB version  #  
#################################

## Approximation with CLT: Example ----
# Waterpolo match: 

# Set the needed information (provided in the RMD report)
p <- 0.5 
q <- 0.7 
n <- m <- 20 

# Obtain mean, variance and standard deviation of W
mW <-  p * n - q * m 
varW <- n * p * (1 - p) + m * q * (1 - q) 
sdW <- sqrt(varW) 

# Compute the probability of interest: P(X > Y) = P(W > 0) = ? 
...
curve(dnorm(x, muw_c, sdw_c), from = 50, to = 130, 
      cex.lab = 2, cex.axis = 2, xlab = "x2", ylab = "")
curve(dnorm(x, muh_c, sdh_c), from = 140, to = 200, 
      cex.lab = 2, cex.axis = 2, xlab = "x1", ylab = "")
